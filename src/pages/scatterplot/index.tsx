...
const Scatterplot = () => {
  ...
  // Fetch scatter plot data using react-query
  ...
  // Watch the file field for displaying the file name
  const fileValue = watch('file');

  // Handle file drop: only accept CSV files
  const handleDrop = (files: File[]) => {
    if (files.length > 0 && files[0].type === 'text/csv') {
      setValue('file', files);
    } else {
      // Optionally clear the file value or show an error message
      setValue('file', []);
    }
  };
