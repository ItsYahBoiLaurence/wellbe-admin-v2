...
const ParticipationRate = ({ selectedDepartment }) => {
  ...
  useEffect(() => {
    if (
      participationData?.participationRate === undefined ||
      participationData?.participationRate === null
    ) {
      ...
      {isParticipationDataLoading ? (
        ...
        <Flex direction={'row'} justify={'space-between'} align={'center'}>
          <Box>
            ...
            <Text fw={500}>
              This summarized the employee's compliance to company activities
              and initiatives.
            </Text>
          </Box>
          {participationLogo == true ? (
            <Text>No data for this department</Text>
          ) : (
            <Flex align={'center'} gap={'md'}>
              <Box>
                <Text fw={700}>Employee Participation</Text>
                <Text size="xl">
                  {Math.floor(participationData?.participationRate)}%
                </Text>
              </Box>
              <RingProgress
                thickness={20}
                sections={[
                  {
                    value: participationData?.participationRate,
                    color: '#FFA903',
                  },
                ]}
              />
